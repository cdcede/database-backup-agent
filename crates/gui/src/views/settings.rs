...
impl SettingsState {
    fn from_config(config: &AppConfig) -> Self {
        let tasks = config.tasks.iter().map(|t| TaskSettingsState {
            name: t.name.clone(),
            databases: t.databases.clone(),
            schedule: t.schedule.clone(),
            retention_days: t.retention_days.to_string(),
        }).collect::<Vec<_>>();
    ...
    fn to_config(&self) -> Result<AppConfig, String> {
        ...
        for (idx, t) in self.tasks.iter().enumerate() {
            ...
            tasks.push(BackupTaskConfig {
                ...
                databases: t.databases.clone(),
                schedule: t.schedule.trim().to_string(),
                retention_days,
            });
        }
...
fn text_input(ui: &mut egui::Ui, text: &mut String, is_password: bool) -> egui::Response {
    let mut edit = egui::TextEdit::singleline(text).desired_width(400.0);
    if is_password {
                        ...
                        ui.vertical(|ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui.add(add_btn).clicked() {
                                    state.tasks.push(TaskSettingsState {
                                        name: format!("Task {}", state.tasks.len() + 1),
                                        databases: Vec::new(),
                            ...
                            if let Some(active_task) = state.tasks.get_mut(state.selected_task_index) {
                                egui::Grid::new("task_editor_grid")
                                    ...
                                    .show(ui, |ui| {
                                        ...
                                        ui.label("Schedule (HH:MM or Cron):");
                                        text_input(ui, &mut active_task.schedule, false);
                                        ui.end_row();
                                        ...
                                        if state.discovered_databases.is_empty() {
                                            ...
                                            ui.vertical(|ui| {
                                                ...
                                                if ui.text_edit_singleline(&mut dbs_str).changed() {
